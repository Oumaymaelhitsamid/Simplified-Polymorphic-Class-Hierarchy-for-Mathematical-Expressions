...
Logarithm::Logarithm(const Expression& F, double l1, double l2, int a) : c1{ l1 }, c2{ l2 }, b{ a }
{
	E = F.clone();
}
...
void Logarithm::display(std::ostream& os) const {
	...
	if (c2 != 0) {
		if (c2 < 0)
		os << " - " << std::fixed << std::setprecision(2) << abs(c2) << " * Log_" << b << "( " << *E << " )";
		else {
			os << " + " << std::fixed << std::setprecision(2) << c2 << " * Log_" << b << "( " << *E << " )";
		}
	}
}
